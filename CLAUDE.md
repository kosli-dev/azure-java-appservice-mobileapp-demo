...
- The `sbom` attestations (`anchore/sbom-action` + `kosli attest custom --type cyclonedx-sbom`,
  added 2026-08-26). Dry-run confirmed `kosli create attestation-type cyclonedx-sbom` (with the
  new `kosli/attestation-types/cyclonedx-sbom.schema.json`, no `--jq`) and a matching
  `kosli attest custom` call against a synthetic CycloneDX file both produce the expected
  payload, and `type: custom:cyclonedx-sbom` is valid per the flow-template JSON schema — but
  `anchore/sbom-action` itself has not run anywhere in this repo, so whether it produces a file
  at `output-file` for a bare zip with no package manifest (the mobile case) is unconfirmed, and
  the type has not actually been created in `kosli-public` yet (needs a bootstrap re-run).
...
`mobileorders-android` and `mobileorders-ios` each get a `sbom` attestation generated by
`anchore/sbom-action` (syft under the hood, CycloneDX JSON) from the built jar or zip and
reported with `kosli attest custom --type cyclonedx-sbom`, bound with `--fingerprint` to the
